...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(130 / 255.f, 132 / 255.f, 170 / 255.f, 0.5f);
    ImVec4 hover_act = ImVec4(147 / 255.f, 157 / 255.f, 249 / 255.f, 1.f);
...
bool ImGui::tab(const char* icon, bool selected)
{
    ...
    ImVec2 label_size =  CalcTextSize(icon) ;
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = { 40, 40 };
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 80 / 255.f, 80 / 255.f, 80 / 255.f, GetStyle().Alpha }, ImVec4{ 144 / 255.f, 150 / 255.f, 196 / 255.f, GetStyle().Alpha }, it_filled->second);

    ImGui::PushStyleColor(ImGuiCol_Text, col);
    ImGui::RenderText(ImVec2(bb.Min.x + (20 - label_size.x / 2) - 5, bb.Min.y + (20 - label_size.y / 2)), icon);
    ImGui::PopStyleColor();

    return pressed;
}
bool ImGui::subtab(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 94 / 255.f, 94 / 255.f, 94 / 255.f, 0.7f * al }, ImVec4{ 48 / 255.f, 119 / 255.f, 233 / 255.f, 1.f * al }, t * al);
    ...
    if (hovered && !selected)
    {
        col = ImVec4(130 / 255.f, 132 / 255.f, 170 / 255.f, 0.5 * al);
        col2 = ImVec4(130 / 255.f, 132 / 255.f, 170 / 255.f, 0.5 * al);
    }
    if (it_filled->second > 0)
        if (!reverse)
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + size.x * min(t * 2, 1), bb.Min.y + 2 * min(t, 1)), ImColor(48, 119, 233, (int)((255 * t) * al)), 12, ImDrawCornerFlags_Bot);
   
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + float((25 + 5.f * t) / 2 - label_size.y / 2)), ImColor(col.x, col.y, col.z, col.w * al), label);
   
    return pressed;
}
bool ImGui::Checkbox(const char* label, bool* v, bool offset)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(10, 8);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.0f * ImGui::GetIO().DeltaTime * ((*v) ? 1.f : -1.4f)), 0.f, 1.f);
    it_filled->second *= g.Style.Alpha;
    ImVec4 text = ImLerp(ImVec4{ 139 / 255.f, 139 / 255.f, 146 / 255.f, g.Style.Alpha }, ImVec4{ 232 / 255.f, 232 / 255.f, 232 / 255.f, g.Style.Alpha }, it_filled->second);
    ImVec4 check = ImLerp(ImVec4{ 80 / 255.f, 80 / 255.f, 80 / 255.f, g.Style.Alpha }, ImVec4{ 144 / 255.f, 150 / 255.f, 196 / 255.f, g.Style.Alpha }, it_filled->second);
    const ImRect check_bb(pos + ImVec2(window->Size.x - 50, 0), pos + ImVec2(window->Size.x - 24, square_sz));
    RenderNavHighlight(total_bb, id);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(14, 12, 15, (int)(g.Style.Alpha * 255)), 10);
    ...
    auto it_pos_active = pos_active.find(id);
    auto it_size_active = size_active.find(id);

    if (it_pos_active == pos_active.end())
    {
        pos_active.insert({ id, 3.f });
        it_pos_active = pos_active.find(id);
    }
    if (it_size_active == size_active.end())
    {
        size_active.insert({ id, 6.f });
        it_size_active = size_active.find(id);
    }
    ...
    if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, ImVec2(check_bb.Min.x + it_pos_active->second, check_bb.Max.y), ImColor(14, 12, 15, (int)(g.Style.Alpha * 255)), 10);
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(g.Style.Alpha * it_pos_active->second, 6), it_size_active->second, ImColor(check), 16);
    }
    else
    {
    ...
    if (label_size.x > 0.0f)
    {
        PushStyleColor(ImGuiCol_Text, text);
...
bool ImGui::RadioButton(const char* label, bool active, const bool theme)
{
    ...
    else
    {
        ...
        if (active)
        {
            window->DrawList->AddCircle(center, radius + 1, ImColor(77, 125, 253, int(t * 255)), 24);
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6,  0), window->DC.CursorPos + ImVec2(0, 0) + ImVec2(window->Size.x - 8, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(10, 10, 10, int(255 * GetStyle().Alpha)), 4);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            if (!popup_open)
            {
                window->DrawList->AddLine(ImVec2(value_x2 + 4, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));
                window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImVec2(value_x2 + 12, frame_bb.Min.y + 5 + 2), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));

                window->DrawList->AddLine(ImVec2(value_x2 + 3, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 11 + 2), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));
                window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 11 + 2), ImVec2(value_x2 + 13, frame_bb.Min.y + 5 + 2), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));
            }
            else
            {
                window->DrawList->AddLine(ImVec2(value_x2 + 4, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        if (CalcTextSize(preview_value).x > 0) {
            if (CalcTextSize(preview_value).x > it_old_value->second)
                it_old_value->second+= 2;
            else if (CalcTextSize(preview_value).x < it_old_value->second)
                it_old_value->second-= 2;
        }
        window->DrawList->PushClipRect(ImVec2(preview_pos), ImVec2(frame_bb.Min.x + style.FramePadding.x + it_old_value->second, frame_bb.Max.y));
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 54, 0.0f), ImVec2(window->Size.x - 54, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(10 / 255.f, 10 / 255.f, 10 / 255.f, 0.9f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = ImClamp(window->Size.x - (window->Flags & ImGuiWindowFlags_Popup ? 24 : 12), 175.f, window->Size.x - (window->Flags & ImGuiWindowFlags_Popup ? 24 : 12));

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6, 2), window->DC.CursorPos + ImVec2(window->Size.x - 8, label_size.y + 18));
    ...
    if ((hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id)
    {
        ...
        SetFocusID(id, window);
        FocusWindow(window);
        g.ActiveIdAllowOverlap = (1 << ImGuiDir_Up) | (1 << ImGuiDir_Down);
    }

    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min + ImVec2(4, 0), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);

    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(4, label_size.y + 4), frame_bb.Max - ImVec2(4, 4));
    ...
    {
        if (it_old_value->second < it_active_value->second)
            it_old_value->second += (it_active_value->second - it_old_value->second) / 10 + 0.6f;
        if (it_old_value->second > it_active_value->second)
            it_old_value->second -= ((it_old_value->second - it_active_value->second) / 10 + 0.6f);
    }
    float scale = (float)percent / 100.f;
    auto it_hover = hover_animation.find(id);
    if (it_hover == hover_animation.end())
    {
        hover_animation.insert({ id, 0.f });
    ...
    it_hover->second = ImClamp(it_hover->second + (4.2f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 1.0f);
    ...
    ImVec4 grip = ImLerp(ImVec4{ 80 / 255.f, 80 / 255.f, 80 / 255.f, g.Style.Alpha }, ImVec4{ 144 / 255.f, 150 / 255.f, 196 / 255.f, g.Style.Alpha }, it_hover->second);

    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(14, 12, 15, (int)(g.Style.Alpha * 255)), 2);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y), ImColor(144 / 255.f, 150 / 255.f, 196 / 255.f, GetStyle().Alpha), 2, ImDrawFlags_RoundCornersLeft);
    ...
    if (g.ActiveId == id || IsItemHovered())
    {
        ...
        {
            it_size->second += 0.1f;
            ImClamp(it_size->second, min_size, max_size);
        }
    }
    else
    {
    ...
    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y - 4), it_size->second, ImColor(80, 80, 80, (int)(g.Style.Alpha * 255)));
    

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    window->DrawList->AddText(ImVec2(slider_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), value_buf);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (CalcTextSize(label).x > 0 && std::string(label).at(0) != '#' && std::string(label).at(1) != '#') {
        SetCursorPosX(28);
        ImGui::Text(label);
    }
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(27, 0), window->DC.CursorPos + ImVec2(window->Size.x - 28, frame_size.y + 0));
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(10, 10, 10), false, 4);
    }
   ...
   // FIXME: We could remove the '&& render_cursor' to keep rendering selection when inactive.
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, ImColor(1.f, 1.f, 1.f, GetStyle().Alpha* ImMax(0.5f, it_hover->second)), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, ImColor(1.f, 1.f, 1.f, GetStyle().Alpha* ImMax(0.5f, it_hover->second)), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImColor t_col = ((selected && hovered) ? ImColor(112, 146, 190) : hovered ? ImColor(177 / 255.f, 177 / 255.f, 177 / 255.f, 0.7f) : selected ? ImColor(112, 146, 190) : ImColor(170, 170, 170, 170));
