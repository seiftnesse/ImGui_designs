...
bool ImGui::tab(const char* icon, const char* label, const char* desc, bool selected, int ssize)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = { float(ssize), 50};

    const ImRect bb(pos, ImVec2(pos.x + ssize, pos.y + size.y));
    ...
    it_hover->second = ImClamp(it_hover->second + (2.15f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 0.4f);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.15f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.2f)), it_hover->second, 1.f);
    it_filled->second *= min(GetStyle().Alpha * 1.5, 1.f);

    static std::map<ImGuiID, float> line_animation;
    auto it_lined = line_animation.find(id);
    if (it_lined == line_animation.end())
    {
        line_animation.insert({ id, 0.f });
        it_lined = line_animation.find(id);
    }
    it_lined->second = ImClamp(it_lined->second + (2.15f * ImGui::GetIO().DeltaTime * (selected && ssize > 220 ? 1.f : -1.5f)), 0.f, 1.f);
    it_lined->second *= min(GetStyle().Alpha * 1.5, 1.f);

    static std::map<ImGuiID, float> main_text_anim;
    auto üפרע_וףקו = main_text_anim.find(id);
    if (üפרע_וףקו == main_text_anim.end())
    {
        main_text_anim.insert({ id, 0.f });
        üפרע_וףקו = main_text_anim.find(id);
    }
    üפרע_וףקו->second = ImClamp(üפרע_וףקו->second + (2.15f * ImGui::GetIO().DeltaTime * (ssize > 220 ? 1.f : -2.8f)), 0.f, 1.f);
    üפרע_וףקו->second *= min(GetStyle().Alpha * 1.5, 1.f);

    static std::map<ImGuiID, float> main_text_anim_act;
    auto act_m = main_text_anim_act.find(id);
    if (üפרע_וףקו == main_text_anim_act.end())
    {
        main_text_anim_act.insert({ id, 0.f });
        act_m = main_text_anim_act.find(id);
    }
    act_m->second = ImClamp(act_m->second + (2.15f * ImGui::GetIO().DeltaTime * (selected && ssize > 220 ? 1.f : -2.8f)), 0.f, 1.f);
    act_m->second *= min(GetStyle().Alpha * 1.5, 1.f);

    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 77 / 255.f, 77 / 255.f, 80 / 255.f, 0.9f }, ImVec4{ 48 / 255.f, 119 / 255.f, 233 / 255.f, 1.f }, it_hover->second);
    ImVec4 col2 = ImLerp(ImVec4{ 122 / 255.f, 122 / 255.f, 122 / 255.f, üפרע_וףקו->second}, ImVec4{ 48 / 255.f, 119 / 255.f, 233 / 255.f, üפרע_וףקו->second }, ImClamp(it_filled->second, 0.f, 1.f));
    ImVec4 col3 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.4f }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 1.f }, t);
   
    if (ssize > 220)
        ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + ssize - 3, bb.Min.y), ImVec2(bb.Min.x + ssize, bb.Max.y), ImColor(48 / 255.f, 119 / 255.f, 233 / 255.f, it_lined->second), 5, ImDrawFlags_RoundCornersLeft);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (42 / 2 - label_size.x / 2), bb.Min.y + (50 / 2 - label_size.y / 2)), ImColor(77 / 255.f, 77 / 255.f, 80 / 255.f, 0.9f), icon);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (42 / 2 - label_size.x / 2), bb.Min.y + (50 / 2 - label_size.y / 2)), ImColor(48 / 255.f, 119 / 255.f, 233 / 255.f, it_filled->second), icon);
    ImGui::PushFont(ImGui::GetIO().Font);
    //if (ssize > 220)
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 45, bb.Min.y + CalcTextSize(label).y / 2), ImColor(col2), label);
   
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 45, bb.Max.y - CalcTextSize(desc).y * 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, üפרע_וףקו->second), desc);
    ImGui::PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float check_sz = ImMin(check_bb.GetWidth() - 2, check_bb.GetHeight() - 2);
    const float pad = ImMax(1.0f, (float)(int)(check_sz / 4.0f));
    RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(51, 51, 51));
   
    window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad - 1, pad - 1), check_bb.Max - ImVec2(pad - 1, pad - 1), ImColor(8 / 255.f, (119 - 40) / 255.f, (233 - 40) / 255.f, it_filled->second), ImColor(8 / 255.f, (119 - 60) / 255.f, (233 - 60) / 255.f, it_filled->second), ImColor(8 / 255.f, (119 - 40) / 255.f, (233 - 40) / 255.f, it_filled->second), ImColor(8 / 255.f, (119 - 80) / 255.f, (233 - 80) / 255.f, it_filled->second));
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(48 / 255.f, (119) / 255.f, (233) / 255.f, it_filled->second));
    if (label_size.x > 0.0f) {
        GetWindowDrawList()->AddText(text_bb.GetTL(), ImColor(220, 220, 220, int(155)), label);
        GetWindowDrawList()->AddText(text_bb.GetTL(), ImColor(220, 220, 220, int(255 * it_filled->second)), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(8,  0), window->DC.CursorPos + ImVec2(0, 0) + ImVec2(window->Size.x - 8, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(55, 55, 55), 2, popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop : ImDrawFlags_::ImDrawFlags_RoundCornersAll);
        //window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(60, 60, 60), 2, popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop : ImDrawFlags_::ImDrawFlags_RoundCornersAll);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
       ...
       // window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, ImColor(180, 180, 180), 2, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 3), ImColor(220, 220, 220), popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.6f);
    }
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 16, 0.0f), ImVec2(window->Size.x - 16, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(frame_bb.Max.x - 2, frame_bb.Max.y - 3 - 8), ImColor(33, 33, 33));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 9), ImColor(55, 55, 55));
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), ImColor(48 / 255.f, (119) / 255.f, (233) / 255.f), 3);
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 2 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 28), GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(window->DC.CursorPos.x + 8, window->DC.CursorPos.y - 32), label);
    Spacing();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
    return value_changed;
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        RenderFrame(bb.Min, ImVec2(bb.Min.x + 5, bb.Max.y), col, false, 0.0f);
