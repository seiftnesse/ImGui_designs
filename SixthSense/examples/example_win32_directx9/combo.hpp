...
namespace QGUI 
{
	...
	bool begincombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
	{
		...
		const ImRect frame_bb(window->DC.CursorPos + ImVec2(20, 22), window->DC.CursorPos + ImVec2(w +0, 40));
		...
		const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
			...
			window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 0), ImVec2(frame_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(0.164706f,0.168627f,0.176471f,1.000000f),0, 15); 
		if (!(flags & ImGuiComboFlags_NoArrowButton))
		{
			ImU32 text_col = ImGui::GetColorU32(ImGuiCol_Text);
				if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
					ImGui::RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y + 0,frame_bb.Min.y + style.FramePadding.y +0), text_col, ImGuiDir_Down,9);
		}

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(229/ 255.f , 229/ 255.f ,229/ 255.f ,255/ 255.f ));
        if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
            ImGui::RenderText(frame_bb.Min + ImVec2(w / 2 - ImGui::CalcTextSize(preview_value).x / 2 - 10, 18 / 2 - ImGui::CalcTextSize(preview_value).y / 2) , preview_value);
		ImGui::PopStyleColor();
		...
		ImGui::SetNextWindowSizeConstraints(ImVec2(w - 20, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
