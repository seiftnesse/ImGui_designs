...
namespace ImGui
{
    ...
    IMGUI_API bool          MenuChild2(const char* str_id, const ImVec2& size_arg = ImVec2(0, 0), bool sub_tab = false, ImGuiWindowFlags extra_flags = NULL);
    IMGUI_API bool          EndMenuChild();
    ...
    bool SubTab(const char* label, bool active, const ImVec2& size_arg);
    // <0 frame_padding uses default frame padding settings. 0 for no padding
    IMGUI_API bool          Checkbox(const char* label, bool* v);
    IMGUI_API bool          tab(const char* icon, const char* label, bool selected, bool arrow);
    bool subtab(const char* label, bool selected);
    bool weapon(const char* label, bool selected, float sizex, int cout, bool reverse);
    bool weapon(const char* label, const ImVec2& size, bool selected);
    bool SubTabEx(const char* label, bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags);
    bool subtabs(const char* label, bool selected);
    bool subtabs(const char* icon, const char* label, bool selected);
    IMGUI_API bool          subtab(const char* label, bool selected, float sizex, int cout = 2, bool reverse = false);
    bool SubTab(const char* label, const ImVec2& size_arg, const bool selected);
    IMGUI_API bool          CheckboxFlags(const char* label, int* flags, int flags_value);
    IMGUI_API bool          CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value);
    IMGUI_API bool          RadioButton(const char* label, bool active, const bool theme = false);                    // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
    IMGUI_API bool          ThemeSwitch(const char* label, bool* v);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, bool text = false);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoDecoration           = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize,
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMultiColorRounded(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding, ImDrawFlags rounding_corners);
    ...
    IMGUI_API void  AddCircularProgressBar(const ImVec2& center, float radius, ImU32 col, float progress, int num_segments = 0, float thickness = 1);
