...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        return ImRect(inner_rect.Min.x + 20, ImMax(outer_rect.Min.y, outer_rect.Max.y - border_size - scrollbar_size), inner_rect.Max.x, outer_rect.Max.y);
    else
        return ImRect(ImMax(outer_rect.Min.x + 0, outer_rect.Max.x - border_size - scrollbar_size + 7), inner_rect.Min.y, outer_rect.Max.x + 3, inner_rect.Max.y);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    it_hover->second = ImClamp(it_hover->second + (1.8f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 0.5f);
    it_hover->second *= ImMin(GetStyle().Alpha * 1.2f, 1.f);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.55f * ImGui::GetIO().DeltaTime * (held ? 1.f : -1.0f)), it_hover->second, 1.f);
    it_filled->second *= ImMin(GetStyle().Alpha * 1.2f, 1.f);
    ImVec4 text = ImLerp(ImVec4{ 50 / 255.f, 50 / 255.f, 52 / 255.f, GetStyle().Alpha }, ImVec4{ 59 / 255.f, 144 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_hover->second);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(text), 4);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(59, 144, 255, int (255 * it_filled->second)), 4);
...
bool ImGui::tab(const char* icon, const char* label, bool selected, bool arrow)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 177, pos.y + 40));
    ...
    ImVec4 text = ImLerp(ImVec4{ 1.f, 1.f, 1.f, 0.4f }, ImVec4{ 220 / 255.f, 220 / 255.f, 220 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImVec4 text2 = ImLerp(ImVec4{ 1.f, 1.f, 1.f, 0.4f }, ImVec4{ 42 / 255.f, 103 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (25 - label_size.x / 2), bb.Min.y + (15 - label_size.y / 2)), ImColor(text), icon);
    ImGui::GetForegroundDrawList()->AddLine(ImVec2(bb.Min.x, bb.Min.y - 7), ImVec2(bb.Min.x + 170, bb.Min.y - 7), ImColor(50, 50, 50, 200), 1.5f);

    if (arrow == true)
    {
        if (selected)
        {
            window->DrawList->AddLine(ImVec2(bb.Max.x - 40, bb.Min.y + 10), ImVec2(bb.Max.x - 35, bb.Min.y + 16), ImColor(text));
            window->DrawList->AddLine(ImVec2(bb.Max.x - 29, bb.Min.y + 9), ImVec2(bb.Max.x - 35, bb.Min.y + 16), ImColor(text));
        }
        else
        {
            window->DrawList->AddLine(ImVec2(bb.Max.x - 35, bb.Min.y + 10), ImVec2(bb.Max.x - 40, bb.Min.y + 16), ImColor(text));
            window->DrawList->AddLine(ImVec2(bb.Max.x - 35, bb.Min.y + 10), ImVec2(bb.Max.x - 29, bb.Min.y + 16), ImColor(text));
        }
    }

    ImGui::PushFont(fontbig);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 50 , bb.Min.y + (15 - CalcTextSize(label).y / 2)), ImColor(text), label);
    ImGui::PopFont();
    return pressed;
}
bool ImGui::subtab(const char* label, bool selected)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(std::string(label).c_str());
    
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = { 177, 25 };

    const ImRect bb(pos, ImVec2(pos.x + 177, pos.y + 25));
    ImGui::ItemSize(size, 0);
    if (!ImGui::ItemAdd(bb, id))
        return false;
    ...
    ImVec4 text = ImLerp(ImVec4{ 1.f, 1.f, 1.f, 0.3f }, ImColor{ 223, 98, 24, 255 }, it_filled->second);

    ImGui::PushFont(font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 50, bb.Min.y + (15 - CalcTextSize(label).y / 2) - 12), ImColor(text), label);
    ImGui::PopFont();
    return pressed;
}

bool ImGui::weapon(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 94 / 255.f, 94 / 255.f, 94 / 255.f, 0.7f * al }, ImVec4{ ImColor( 223, 98, 24, 255) }, t * al);
    ImVec4 col2 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.4f * al }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 0.5f * al }, t * al);
    ImVec4 col3 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.4f * al }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 1.f * al }, t * al);


    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + 5), ImColor(text), label);

    if (selected)
        ImGui::GetWindowDrawList()->AddLine(ImVec2(bb.Min.x, bb.Max.y - 1), ImVec2(bb.Min.x + size.x, bb.Max.y - 1), ImColor{ 223, 98, 24, 255 }, 1.f);

    return pressed;

}

bool ImGui::SubTabEx(const char* label, bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec4 hover;
    if (*active)
        hover = ImColor(71, 89, 177, 255);
    else
        hover = ImColor(83, 75, 151, 0);


    if (*active)
        GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(bb.Min.x + 35, bb.Max.y - 7), ImVec2(bb.Max.x - 37, bb.Max.y), ImColor(71, 89, 177, 0), ImColor(71, 89, 177, 0), ImColor(71, 89, 177, 190), ImColor(71, 89, 177, 190));

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x + 35, bb.Max.y - 1), ImVec2(bb.Max.x - 37, bb.Max.y), GetColorU32(hover));
    if (*active)
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + 15), ImVec2(bb.Max.x, bb.Max.y), label, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);;
        PopStyleColor();
    }
    else
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.3f));
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + 15), ImVec2(bb.Max.x, bb.Max.y), label, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);;
        PopStyleColor();
    }
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
...
bool ImGui::SubTab(const char* label, bool active, const ImVec2& size_arg)
{
    return SubTabEx(label, &active, size_arg, 0);
}

std::map<ImGuiID, float> checkbox;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(0, 3), check_bb.Max - ImVec2(6, 3), ImColor(17, 16, 17, (int)(g.Style.Alpha * 255)), 2.f);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(0, 3), check_bb.Max - ImVec2(6, 3), GetColorU32(ImVec4(223 / 255.f, 98 / 255.f, 24 / 255.f, *alpha)), 2.f);

    const float pad = ImMax(1.0f, (float)(int)(square_sz / 4.0f));
    RenderCheckMark(check_bb.Min + ImVec2(pad - 0.5f, pad + 1.7f), GetColorU32(ImVec4(1.f, 1.f, 1.f, *alpha)), square_sz - pad * 2.0f - 5.f);

    if (*v)
    {
        if (label_size.x > 0.0f)
            RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x - 2.f, check_bb.Min.y + style.FramePadding.y + 1), label);
    }
    else
    {
...
bool ImGui::RadioButton(const char* label, bool active, const bool theme)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!ItemAdd(total_bb, id))
        return false;
    float t = active ? 1.0f : 0.0f;
    float ANIM_SPEED = 0.99f; // Bigger = Slower
    if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
    ...
    {
        auto lb = label == "##black";
        window->DrawList->AddCircleFilled(center - ImVec2(0, 3), radius, lb ? ImColor(3, 16, 29) : ImColor(244, 244, 244), 24);
        if (active)
        {
            window->DrawList->AddCircle(center, radius + 1, ImColor(71, 89, 177, int(t * 255)), 24);
        }
    }
...
//    const float square_sz = GetFrameHeight();
    ...
    if (label_size.x > 0.0f)
        window->DrawList->AddText(label_pos - ImVec2(30, 0), ImColor(220, 220, 220, 220),  label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#') {
        ImGui::SetCursorPosX(15);
        PushStyleColor(ImGuiCol_Text, ImVec4(73 / 255.f, 73 / 255.f, 73 / 255.f, GetStyle().Alpha));
        ImGui::Text(label);
        PopStyleColor();
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max + ImVec2(0, 10), ImColor(27, 26, 27, int(255 * GetStyle().Alpha)), 4);
     }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x) {
            window->DrawList->AddLine(ImVec2(value_x2 + 4- 6, frame_bb.Min.y + 10 + 3), ImVec2(value_x2 + 8- 6, frame_bb.Min.y + 14 + 3), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
            window->DrawList->AddLine(ImVec2(value_x2 + 8- 6, frame_bb.Min.y + 14 + 3), ImVec2(value_x2 + 12- 6, frame_bb.Min.y + 9 + 3), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));

            window->DrawList->AddLine(ImVec2(value_x2 + 3- 6, frame_bb.Min.y + 10 + 3), ImVec2(value_x2 + 8- 6, frame_bb.Min.y + 15 + 3), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
            window->DrawList->AddLine(ImVec2(value_x2 + 8- 6, frame_bb.Min.y + 15 + 3), ImVec2(value_x2 + 13- 6, frame_bb.Min.y + 9 + 3), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
        }

    }

    RenderFrameBorder(frame_bb.Min, frame_bb.Max + ImVec2(0, 10), style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ImVec2 preview_pos = frame_bb.Min + ImVec2(style.FramePadding.x + 5, (16 - CalcTextSize(preview_value).y / 2));
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = ImClamp(window->Size.x - 15, 155.f, window->Size.x - 15);

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w - 22, label_size.y + 14));
    ...
    if ((hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id)
    {
        ...
        SetFocusID(id, window);
        FocusWindow(window);
        //g.activeidallow = (1 << ImGuiDir_Up) | (1 << ImGuiDir_Down);
    }

    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min, ImColor(255, 255, 255, 120), label);

    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(0, label_size.y + 4), frame_bb.Max - ImVec2(0, 4));

    int percent = 0;
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(slider_bb, id, data_type, p_data, p_min, p_max, format, ImGuiSliderFlags_None, &grab_bb, &percent);
    if (value_changed)
        MarkItemEdited(id);

    //static float size = 5.f;
    static float min_size = 6.f;
    static float max_size = 6.f;
    ...
    if (g.ActiveId == id || IsItemHovered())
    {
        ...
        {
            it_size->second += 0.1f;
            clamp(it_size->second, min_size, max_size);
        }
    }
    else
    {
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(17, 16, 17, (int)(g.Style.Alpha * 255)), 2);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), ImColor{ 223, 98, 24, 255 });

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y - 3), 6.f, ImColor(255, 255, 255, 255), 32);
    ...
    const ImVec4 hover_dis = ImVec4(102 / 255.f, 153 / 255.f, 204 / 255.f / 255.f, g.Style.Alpha);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
    ...
    window->DrawList->AddText(ImVec2(slider_bb.Max.x - 2 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 33), GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if ((flags & ImGuiInputTextFlags_CallbackHistory) != 0 && IsKeyPressedMap(ImGuiKey_UpArrow))
                {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;
    ImColor col = ((selected && hovered) ? ImColor(112, 146, 190) : hovered ? ImColor(112, 146, 190, 100) : ImColor(112, 146, 190));
    if (hovered || selected)
    {
    ...
    ImColor t_col = ((selected && hovered) ? ImColor( 223, 98, 24, 255) : hovered ? ImColor(177 / 255.f, 177 / 255.f, 177 / 255.f, 0.7f * g.Style.Alpha) : selected ? ImColor(223, 98, 24, 255) : ImColor(170, 170, 170, int(170 * g.Style.Alpha)));
