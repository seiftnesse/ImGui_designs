...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0, 0.5), &bb);
...
bool ImGui::TabEx(const char* icon, const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (active) {
        ...
        d->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(255, 90, 90, alpha / 5), ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 5));
        d->AddRectFilled(ImVec2(bb.Max.x - 2, bb.Min.y), bb.Max, ImColor(255, 90, 90, alpha));
    }
    ImColor col = active ? GetColorU32(ImGuiCol_Text) : GetColorU32(ImGuiCol_TextDisabled);

    PushStyleColor(ImGuiCol_Text, (ImVec4)col);

    PushFont(g_IconsFont);
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(30, 0), bb.Max - style.FramePadding, icon, NULL, &icon_size, ImVec2(0.f, 0.15f), &bb);
    PopFont();

    PushFont(g_TabsFont);
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(60, 0), bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.f, 0.5f), &bb);
    PopFont();
...
bool ImGui::SubTabEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (active) {
        auto d = ImGui::GetWindowDrawList();
        d->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 5), ImColor(255, 90, 90, alpha / 5));
        d->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), bb.Max, ImColor(255, 90, 90, alpha));
    }
    ImColor col = active ? GetColorU32(ImGuiCol_Text) : GetColorU32(ImGuiCol_TextDisabled);
...
bool ImGui::SubTab(const char* label, const bool active, const ImVec2& size_arg)
{
...
bool ImGui::Bind(const char* label, int* key, const ImVec2& size_arg)
{
    ...
    const float ws = GetWindowSize().x;
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(ws - size.x - style.ItemSpacing.x * 2, 0), window->DC.CursorPos + ImVec2(ws - style.ItemSpacing.x * 2, size.y));
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text, alpha));
    ...
    if (label_size.x > 0)
        RenderTextClipped(total_bb.Min, frame_bb.Max - style.FramePadding, label, NULL, NULL, ImVec2(0, style.ButtonTextAlign.y));
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    RenderArrow(window->DrawList, bb.Min + g.Style.FramePadding, text_col, 1.0f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBgActive, it_filled->second), true, style.FrameRounding);
    ...
    ImU32 active_col = GetColorU32(ImGuiCol_Text);
    ImU32 hovered_col = GetColorU32(ImGuiCol_Text, it_hover->second);
    ImU32 disabled_col = GetColorU32(ImGuiCol_TextDisabled);

    PushStyleColor(ImGuiCol_Text, disabled_col);
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label);
    PushStyleColor(ImGuiCol_Text, *v ? active_col : hovered_col);
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label);
    PopStyleColor(2);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float frame_size = ws.x - style.ItemInnerSpacing.x * 3.5f;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(frame_size, label_size.y + style.FramePadding.y * 4.0f));
    ...
    static std::map<ImGuiID, float> light_animation;
    auto it_light = light_animation.find(id);
    if (it_light == light_animation.end())
    {
        light_animation.insert({ id, 0.f });
        it_hover = light_animation.find(id);
    }
    it_light->second = clamp(it_light->second + (3.f * ImGui::GetIO().DeltaTime * (hovered ? 0.3f : -1.f)), 0.0f, 0.3f);
    ...
    it_open->second = clamp(it_open->second + (1.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.01f, 1.f);

    window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), GetColorU32(ImGuiCol_FrameBgActive, it_light->second), style.FrameRounding, ImDrawCornerFlags_All);
    window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_FrameBgActive, it_hover->second), style.FrameRounding, ImDrawCornerFlags_All);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), style.FrameRounding, ImDrawCornerFlags_All);
    RenderArrow(window->DrawList, ImVec2(frame_bb.Max.x - 20, frame_bb.Min.y), GetColorU32(ImGuiCol_Text), 15);
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + ImVec2(style.FramePadding.x * 3, style.FramePadding.y), frame_bb.Max - style.FramePadding, preview_value, NULL, NULL, ImVec2(0.f, 0.5f));
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y * 7), label);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(frame_size, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    RenderNavHighlight(frame_bb, id);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 4), GetColorU32(ImGuiCol_FrameBg), true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(1, 4), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(1, 5), GetColorU32(ImGuiCol_FrameBgActive, 0.3f), true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 4), GetColorU32(ImGuiCol_FrameBgActive), true, 5.f);
    ...
    std::string value = " [";
    value.append(value_buf); value.append("/"); value.append(std::to_string(v_max)); value.append("]");
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const ImVec2 padding = (display_frame || (flags & ImGuiTreeNodeFlags_FramePadding)) ? style.FramePadding : ImVec2(style.FramePadding.x, ImMin(window->DC.CurrentLineTextBaseOffset, style.FramePadding.y));
    ...
    const float text_offset_y = ImMax(padding.y, window->DC.CurrentLineTextBaseOffset);                    // Latch before ItemSize changes it
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, 1.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, 0.70f);
        if (g.LogEnabled)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    {
        auto d = ImGui::GetWindowDrawList();
        d->AddRectFilled(ImVec2(bb.Max.x - 4, bb.Min.y), bb.Max, ImColor(255, 90, 90, 150));
    }
    if (selected)
    {
        auto d = ImGui::GetWindowDrawList();
        d->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(255, 90, 90, alpha / 5), ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 50), ImColor(255, 90, 90, alpha / 5));
        d->AddRectFilled(ImVec2(bb.Max.x - 4, bb.Min.y), bb.Max, ImColor(255, 90, 90, alpha));
    ...
    ImColor col = GetColorU32(selected ? ImGuiCol_Text : hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled);
    PushStyleColor(ImGuiCol_Text, (ImVec4)col);
    RenderTextClipped(text_min + ImVec2(3, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
