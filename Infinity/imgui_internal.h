...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CurrLineSize;
    ImVec2                  PrevLineSize;
    float                   CurrentLineTextBaseOffset; // Baseline offset (0.0f by default on a new line, generally == style.FramePadding.y when a framed item has been added).
...
namespace ImGui
{
    ...
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float scale = 1.0f);
    ...
    inline void RenderArrow(ImVec2 pos, float scale=1.0f) { ImGuiWindow* window = GetCurrentWindow(); RenderArrow(window->DrawList, pos, GetColorU32(ImGuiCol_Text), scale); }
    inline void RenderBullet(ImVec2 pos)                                { ImGuiWindow* window = GetCurrentWindow(); RenderBullet(window->DrawList, pos, GetColorU32(ImGuiCol_Text)); }
#endif

    /*                     INFINITY GUI                     */
    IMGUI_API bool          TabEx(const char* icon, const char* label, const bool active = false, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
	IMGUI_API bool          SubTabEx(const char* label, const bool active = false, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
    /*                     INFINITY GUI                     */
