...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled( bb.Min, bb.Max, hovered ? ImColor( 30, 30, 30, 255 ) : ImColor( 51, 51, 51, 255 ), 2.f );
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    if (held)
        window->ScrollNext.y = ImGui::GetScrollY();
}
...
bool ImGui::Checkbox(const char* label, bool* v) 
{
    ...
    window->DrawList->AddRect( check_bb.Min + ImVec2( 0, 3 ), check_bb.Max - ImVec2( 6, 3 ), hovered ? ImColor( 255, 255, 255, 255 ) : ImColor( 153, 153, 153, 255 ), 2.f );
 
        window->DrawList->AddRectFilled( check_bb.Min + ImVec2( 0, 3 ), check_bb.Max - ImVec2( 6, 3 ), GetColorU32( ImVec4( 0.f, 120 / 255.f, 215 / 255.f, *alpha  ) ), 2.f );

        const float pad = ImMax(1.0f, (float)(int)(square_sz / 4.0f));
        RenderCheckMark(check_bb.Min + ImVec2(pad - 1.7f, pad + 0.7f), GetColorU32( ImVec4( 1.f, 1.f, 1.f, *alpha ) ), square_sz - pad * 2.0f - 2.f);
    

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x - 2.f, check_bb.Min.y + style.FramePadding.y - 1.f), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRect(ImVec2(frame_bb.Min.x , frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), hovered ? ImColor(255, 255, 255, 255) : ImColor(153, 153, 153, 255), 2.f);

    if (!(flags & ImGuiComboFlags_NoArrowButton))
        window->DrawList->AddText(icon, 12.f, ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 2.f), ImColor(255, 255, 255, 255), "_");

    if (popup_open)
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(51, 51, 51, 255), 2.f, ImDrawCornerFlags_Top);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + style.FramePadding.y * 2.0f), window->DC.CursorPos + ImVec2(w, label_size.y * 2 + style.FramePadding.y * 2.0f + 6.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0.f, label_size.y > 0.0f ? style.FramePadding.y + label_size.y * 1.f : 0.f));
    ...
    if (value_changed)
        MarkItemEdited(id);
    // Draw frame
    RenderFrame(frame_bb.Min + ImVec2( 0, 8 ), frame_bb.Max - ImVec2( 2, 13 ), ImColor( 153, 153, 153, 255 ), true, 0.f);
    RenderFrame(frame_bb.Min + ImVec2( 0, 8 ), grab_bb.Max - ImVec2( 2, 11 ), ImColor( 0, 120, 215, 255 ), true, 0.f);


    // Render grab
    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    window->DrawList->AddCircleFilled( grab_bb.Min + ImVec2(2, 7), 9, ImColor( 0, 120, 215, 255 ), 360 );
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    auto text_size = ImGui::CalcTextSize(value_buf, value_buf_end, true);

    float* alpha = &slider[id];
    ...
    window->DrawList->AddRectFilled(grab_bb.Min - ImVec2(15, 30), grab_bb.Max + ImVec2(text_size.x, -25), GetColorU32(ImVec4(51 / 255.f, 51 / 255.f, 51 / 255.f, *alpha)), 2.f);
    PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, *alpha));
    RenderTextClipped(grab_bb.Min - ImVec2(15, 28), grab_bb.Max + ImVec2(text_size.x, -28), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, hovered ? ImColor(255, 255, 255, 255) : ImColor(153, 153, 153, 255), true, 2.f);
    }
