...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(120,120,120,120), 4,15,2);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, it_circle->second.alpha)), 60);
    window->DrawList->PopClipRect();

    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, it_hover->second)), 2.f );
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + 155.f, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const float pad = ImMax(3.0f, (float)(int)(check_sz / 6.0f));

    if (*v)
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 3), ImVec2(check_bb.Max.x, check_bb.Max.y - 2), ImColor(0.f / 255.f, 92.f / 255.f, 253.f / 255.f, 255.f / 255.f), 24.f);
    else
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 3), ImVec2(check_bb.Max.x, check_bb.Max.y - 2), ImColor(79.f / 255.f, 78.f / 255.f, 74.f / 255.f, 255.f / 255.f), 24.f);
    ...
    if (label_size.x > 0.0f)
    {
        ...
        if (*v)
        {
            ...
            RenderText(ImVec2(check_bb.Min.x + 65, check_bb.Min.y + style.FramePadding.y), label);
            ImGui::PopStyleColor();
        }
        else
        {
            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.58f, 0.58f, 0.58f, 1.f));
            RenderText(ImVec2(check_bb.Min.x + 65, check_bb.Min.y + style.FramePadding.y), label);
            ImGui::PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    col_bg2 = ImGui::GetColorU32(ImVec4(5 / 255.f, 193 / 255.f, 245 / 255.f, 255 / 255.f));

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, col_bg, 3.f, 15,2.f);
    if(hovered)
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 1,1 }, frame_bb.Max - ImVec2{ 1, 1 }, col_bg2, 3.f);

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - 20), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
 ...
 // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 0,7 }, frame_bb.Max - ImVec2{ 12,3 }, ImColor(19, 22, 26), 6.f);
    ...
    col_bg = ImGui::GetColorU32(ImLerp(ImVec4(15 / 255.f, 193 / 255.f, 245 / 255.f, 255.f / 255.f), ImVec4(5 / 255.f, 193 / 255.f, 245 / 255.f, 255.f / 255.f), t));


    // Render grab
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 0,7 }, ImVec2{ grab_bb.Min.x,grab_bb.Max.y - 1 }, col_bg, 6.f);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 9), label);


    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x - 12, frame_bb.Min.y - 9), value_buf);
...
bool ImGui::ColorEdit(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImVec4 col_v4(f[0], f[1], f[2], alpha ? f[3] : 1.0f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), 4.f, ImDrawCornerFlags_TopRight| ImDrawCornerFlags_BotRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 4.f, ImDrawCornerFlags_TopLeft|ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 4.f);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 4.f, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(120,120,120,0), 4.f); // Color button are often in need of some sort of border
    }
