...
extern ImFont* g_pTabsFont;
bool ImGui::ButtonActiveEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    const ImU32 col2 = GetColorU32(active ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
    RenderNavHighlight(bb, id);
    if (active)
    {
        RenderFrame(bb.Min, bb.Max, col2, true, style.FrameRounding);
...
bool ImGui::ButtonActive(const char* label, const bool active, const ImVec2& size_arg)
{
    return ButtonActiveEx(label, active, size_arg,  ImGuiButtonFlags_None);
}
bool ImGui::SubTabEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (active || hovered)
    {
        PushStyleColor(ImGuiCol_Text, (ImVec4)ImColor(85, 129, 241));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
    else
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.x + 42.f, label_size.y + 3.f));

    ItemSize(check_bb, style.FramePadding.y); ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos, window->DC.CursorPos);

    if (label_size.x > 0) {

        ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight()), style.FramePadding.y);
        total_bb = ImRect(ImMin(check_bb.Min, text_bb.Min), check_bb.Max);
    }
    ...
    if (*v)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y+3), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 13), ImColor(20, 20, 20), 12.f);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 15, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16), ImColor(85, 129, 241), 12.f);
    }
    else
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y + 3), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 13), ImColor(20, 20, 20), 12.f);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 15, check_bb.Min.y + 16), ImColor(33, 33, 33), 12.f);
    }
    if (label_size.x > 0.0f) RenderText(check_bb.Min + ImVec2(36.f, -1.1f), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
        {
            if(!popup_open)
                ...
                RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Up, 1.0f);
        }
    }
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, preview_value, NULL, NULL, ImVec2(0.5f, 0.0f));
    if (label_size.x > 0)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 6.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    RenderNavHighlight(frame_bb, id);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), frame_col, true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(85, 129, 241, 200), true, 5.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 5.f, ImColor(85, 129, 241), 64);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y*1), label);
