...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(pos + ImVec2(22, 0), pos + size + ImVec2(22, 0));
    ItemSize(size + ImVec2(0, 6), style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    PushFont(three);
    PushStyleColor(ImGuiCol_Text, ImVec4(0.74f, 0.7f, 0.81f, 1.f));
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(0, 1), bb.Max - style.FramePadding + ImVec2(0, 1), label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleColor();
    PopFont();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.0f) * 0.5f), 0.0f, 8.0f), -ImClamp(IM_FLOOR((bb_frame_height - 2.0f) * 0.5f), 0.0f, 3.0f)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect checkbox_bb(pos, pos + ImVec2(square_sz + style.ItemInnerSpacing.x - 9, style.FramePadding.y * 2.0f + 9));
    const ImRect total_bb(pos, pos + ImVec2(square_sz + style.ItemInnerSpacing.x - 9, style.FramePadding.y * 2.0f + 12));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(checkbox_bb, id))
    {
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(checkbox_bb, id, &hovered, &held);
    if (pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(checkbox_bb.Min, checkbox_bb.Max, ImColor(159, 82, 89), 2.f);
        window->DrawList->AddRectFilledMultiColor(checkbox_bb.Min + ImVec2(1, 1), checkbox_bb.Max - ImVec2(1, 1), ImColor(172, 102, 106), ImColor(188, 84, 98), ImColor(193, 66, 100), ImColor(171, 84, 100));
        RenderCheckMark(window->DrawList, total_bb.Min + ImVec2(3, 3), ImColor(255, 255, 255), 8.f);
    }
    else if (hovered)
    {
        window->DrawList->AddRectFilled(ImVec2(checkbox_bb.Min), ImVec2(checkbox_bb.Max), ImColor(40, 28, 54), 2.f);
        window->DrawList->AddRect(ImVec2(checkbox_bb.Min), ImVec2(checkbox_bb.Max), ImColor(64, 53, 72), 2.f);
    }
    else
    {
        window->DrawList->AddRectFilled(ImVec2(checkbox_bb.Min), ImVec2(checkbox_bb.Max), ImColor(30, 19, 44), 2.f);
        window->DrawList->AddRect(ImVec2(checkbox_bb.Min), ImVec2(checkbox_bb.Max), ImColor(64, 53, 72), 2.f);
    }

    window->DrawList->AddText(three, 12.f, checkbox_bb.Min + ImVec2(23, 2), ImColor(255, 255, 255), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect combo_bb(window->DC.CursorPos + ImVec2(22, 16), window->DC.CursorPos + ImVec2(w + 49, style.FramePadding.y * 2.0f + 35));
    const ImRect total_bb(combo_bb.Min - ImVec2(0, 16), combo_bb.Max + ImVec2(style.ItemInnerSpacing.x - 4, 6.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &combo_bb))
        return false;

    // Open on click
    bool hovered, held;
    bool pressed = ButtonBehavior(combo_bb, id, &hovered, &held);
    ...
    static std::map<ImGuiID, float> open;
    auto anim = open.find(id);
    if (anim == open.end())
    {
        open.insert({ id, 0.f });
        anim = open.find(id);
    ...
    if (popup_open)
        anim->second = ImClamp(anim->second + (4 * ImGui::GetIO().DeltaTime * ((popup_open) ? 1.f : -1.f)), 0.f, 1.f);
    else if (!popup_open)
        anim->second = ImClamp(anim->second + (4 * ImGui::GetIO().DeltaTime * ((popup_open) ? 1.f : -1.f)), 0.f, 1.f);

    // Render shape
   // window->DrawList->AddRectFilled(ImVec2(total_bb.Min), ImVec2(total_bb.Max), ImColor(0, 0, 0));
    window->DrawList->AddRectFilled(ImVec2(combo_bb.Min), ImVec2(combo_bb.Max), ImColor(30, 19, 42), 2.f);
    window->DrawList->AddRectFilled(combo_bb.Max - ImVec2(26, 23), combo_bb.Max - ImVec2(1, 1), ImColor(30, 19, 42), 2.f, ImDrawFlags_RoundCornersRight);
    window->DrawList->AddRect(ImVec2(combo_bb.Min), ImVec2(combo_bb.Max), ImColor(65, 54, 81), 2.f);

    if (popup_open) {
        window->DrawList->AddTriangleFilled(combo_bb.Max - ImVec2(18, 10), combo_bb.Max - ImVec2(10, 10), combo_bb.Max - ImVec2(14, 14), ImColor(149, 138, 162));
    }


    if (!popup_open) {
        window->DrawList->AddTriangleFilled(combo_bb.Max - ImVec2(18, 15), combo_bb.Max - ImVec2(10, 15), combo_bb.Max - ImVec2(14, 10), ImColor(149, 138, 162));
        anim->second = 0.f;
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        PushFont(three);
        PushStyleColor(ImGuiCol_Text, ImVec4(156.f / 255.f, 145.f / 255.f, 170.f / 255.f, 1.f));
        RenderTextClipped(combo_bb.Min + ImVec2(10, 0), combo_bb.Max - ImVec2(26, 0), preview_value, NULL, NULL, ImVec2(0.0f, 0.5f));
        PopStyleColor();
        PopFont();
    }

    window->DrawList->AddText(three, 12.f, total_bb.Min + ImVec2(0, 1), ImColor(156, 145, 170), label);

    if (!popup_open)
        return false;

    g.NextWindowData.Flags = backup_next_window_data_flags;
    return BeginComboPopup(popup_id, anim->second ,combo_bb, flags);
}

bool ImGui::BeginComboPopup(ImGuiID popup_id, float size, const ImRect& bb, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f * size), ImVec2(FLT_MAX, ((CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * size)));
    }
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(8, 4)); // Horizontally align ourselves with the framed text
    PushStyleColor(ImGuiCol_Border, ImVec4(65.f / 255.f, 54.f / 255.f, 79.f / 255.f, 1.f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect grab_bbb(window->DC.CursorPos + ImVec2(16, 14), window->DC.CursorPos + ImVec2(w + 53, 24));
    const ImRect slider_bb(window->DC.CursorPos + ImVec2(23, 16), window->DC.CursorPos + ImVec2(w + 48, 24));
    const ImRect total_bb(slider_bb.Min - ImVec2(0, 13), slider_bb.Max + ImVec2(style.ItemInnerSpacing.x - 4, 10.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &grab_bbb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    if (temp_input_is_active)
    {
        ...
        return TempInputScalar(grab_bbb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);
    }

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(grab_bbb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

   // window->DrawList->AddRectFilled(ImVec2(total_bb.Min), ImVec2(total_bb.Max), ImColor(255, 255, 255));
    window->DrawList->AddRectFilledMultiColor(slider_bb.Min + ImVec2(1, 1), slider_bb.Max - ImVec2(1, 1), ImColor(177, 96, 88), ImColor(171, 71, 103), ImColor(171, 71, 103), ImColor(177, 96, 88));

    window->DrawList->AddRectFilled(grab_bb.Min + ImVec2(6, 0), ImVec2(slider_bb.Max), ImColor(30, 19, 44), 2.f, ImDrawFlags_RoundCornersLeft);
    window->DrawList->AddRect(ImVec2(slider_bb.Min), ImVec2(slider_bb.Max), ImColor(64, 53, 72), 2.f);

    window->DrawList->AddText(three, 12.f, total_bb.Min - ImVec2(0, 2), ImColor(148, 135, 162), label);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    ImGui::PushFont(three);
    RenderTextClipped(ImVec2(grab_bb.Min.x - 100, grab_bb.Min.y - 9), ImVec2(grab_bb.Max.x + 100, grab_bb.Min.y + 25), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ImGui::PopFont();
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(size_arg.x != 0.0f ? size_arg.x : label_size.x, size_arg.y != 0.0f ? size_arg.y : label_size.y + 5);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset;
    ItemSize(size, 0.0f);
    ...
    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;
    PushFont(three);
    PushStyleColor(ImGuiCol_Text, ImVec4(150.f / 255.f, 139.f / 255.f, 162.f / 255.f, 1.f));
    RenderTextClipped(text_min + ImVec2(0, 2), text_max + ImVec2(0, 2), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
    PopFont();
    ...
    {
        PushFont(three);
        PushStyleColor(ImGuiCol_Text, ImVec4(162.f / 255.f, 101.f / 255.f, 112.f / 255.f, 1.f));
        RenderTextClipped(text_min + ImVec2(0, 2), text_max + ImVec2(0, 2), label, NULL, &label_size, style.SelectableTextAlign, &bb);
        PopStyleColor();
        PopFont();
    }
...
bool ImGui::Навалил_Кринжа(const char* label, bool selected, float SHIRINA)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(SHIRINA, 31), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (selected)
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(41, 29, 55));
        PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
        RenderTextClipped(bb.Min, bb.Max, label, NULL, NULL, ImVec2(0.5f, 0.5f));
        PopStyleColor();
    }
    else
    {
        if (!hovered)
        {
            window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(23, 17, 27));
            PushStyleColor(ImGuiCol_Text, ImVec4(0.56f, 0.52f, 0.61f, 1.f));
            RenderTextClipped(bb.Min, bb.Max, label, NULL, NULL, ImVec2(0.5f, 0.5f));
            PopStyleColor();
        }
        else
        {
            window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(30, 19, 42));
            PushStyleColor(ImGuiCol_Text, ImVec4(0.73f, 0.7f, 0.79f, 1.f));
            RenderTextClipped(bb.Min, bb.Max, label, NULL, NULL, ImVec2(0.5f, 0.5f));
            PopStyleColor();
        }
    }
    PopFont();
    window->DrawList->AddRect(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(66, 55, 82));


    return pressed;
}

void ImGui::MenuChild(const char* str_id, const ImVec2& size, const ImVec2& pos, ImGuiWindowFlags extra_flags) {
    ImGui::SetCursorPos(pos);
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(15, 12));
    ImGui::BeginChild(str_id, size, false, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding);
    ImGui::BeginGroup();
    PopStyleVar();
