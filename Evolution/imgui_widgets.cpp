...
{
    auto aslpha = float(GetStyle().Alpha);
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(8, 0);
    ...
    it_hover->second = clamp(it_hover->second + (0.2f * GetIO().DeltaTime * (hovered ? 1.f : -5.f)), 0.0f, 0.12f);
    ...
    it_filled->second = clamp(it_filled->second + (2.35f * GetIO().DeltaTime * ((*v) ? 1.f : -2.f)), it_hover->second, 1.f);
    ...
    window->DrawList->AddRect(ImVec2(check_bb.Min.x + pad - 1, check_bb.Min.y + pad * 1.4 - 1), ImVec2(check_bb.Max.x - pad + 1, check_bb.Max.y - 2), ImColor(60, 60, 60, int(alpha * aslpha)), 2);

    if (*v || it_filled->second > 0.01)
    {
        auto g = int((it_filled->second * 255) * aslpha);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + pad, check_bb.Min.y + pad * 1.4), ImVec2(check_bb.Max.x - pad, check_bb.Max.y - 3), ImColor(77, 125, 253, g), 2);
        RenderCheckMark(window->DrawList, ImVec2(check_bb.Min.x + pad + 2, check_bb.Min.y + pad * 1.4 + 2), ImColor(0, 0, 0, g), 6);
    }
    if ((hovered || it_hover->second > 0.01))
    {
        auto g = int(255 * it_hover->second);
        RenderCheckMark(window->DrawList, ImVec2(check_bb.Min.x + pad + 2, check_bb.Min.y + pad * 1.4 + 2), ImColor(255, 255, 255, g), 6);
    }

    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        window->DrawList->AddText(check_bb.Min + ImVec2(pad, pad) + ImVec2(18, 0), ImColor(200, 200, 200, int((100 * aslpha) + (155 * it_filled->second))), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    SetCursorPosX(10);
    TextColored(ImVec4(200 / 255.f, 200 / 255.f, 200 / 255.f, 1.f), label);
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 0), window->DC.CursorPos + ImVec2(window->Size.x - 10, label_size.y + style.FramePadding.y * 2.0f));
    ...
    const ImVec4 hover_dis = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.4f);;
    float deltatime = 1.5f * GetIO().DeltaTime;
    ...
    const ImVec4 arrow_act = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(11, 11, 11, int(255 * GetStyle().Alpha)), 4);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(24, 24, 24), 4);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        // window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, ImColor(180, 180, 180), 2, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 3), ImColor(it_arrow_col->second), popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.6f);
    }

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ImVec2 preview_pos = frame_bb.Min + style.FramePadding;
        window->DrawList->AddText(preview_pos, ImColor(174, 174, 182), preview_value, NULL);
    }
    ...
    it_open->second = clamp(it_open->second + (3.f * GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.01f, 1.f);
    ...
    if (it_open->second < 0.99f) {
        PushStyleVar(ImGuiStyleVar_ScrollbarSize, 0.1f);
        GetStyle().ScrollbarRounding = 0;
    }
    else
    {
        if (GetStyle().ScrollbarRounding < 1)
            GetStyle().ScrollbarRounding += 0.02;
    }
    if (!popup_open)
        return false;
    PushStyleVar(ImGuiStyleVar_Alpha, it_open->second * GetStyle().Alpha);
    PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupBorderSize, 0);
    PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 4);
    PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_WindowPadding, ImVec2(2, 4));
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(15 / 255.f, 15 / 255.f, 15 / 255.f, it_open->second * 0.8f));
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 20, 0.0f), ImVec2(window->Size.x - 20, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * it_open->second));
    }
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(0, 1));
        }
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    if (popup_max_height_in_items != -1 && !(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint))
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 20, 0.0f), ImVec2(window->Size.x - 20, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 20), window->DC.CursorPos + ImVec2(GetWindowSize().x - 10, 28));
    const ImRect total_bb(ImVec2(frame_bb.Min.x - 2, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 7, frame_bb.Max.y + 25));
    ...
    it_hover->second = clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    it_filled->second = clamp(it_filled->second + (1.15f * GetIO().DeltaTime), 0.0f, 1.f);
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 8), ImColor(11, 11, 11, int(255 * GetStyle().Alpha)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 2), ImVec2(max(frame_bb.Min.x, grab_bb.Min.x * it_filled->second * GetStyle().Alpha), frame_bb.Min.y + 8), ImColor(77, 125, 253, int(255 * GetStyle().Alpha)), 4);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    auto size_text = CalcTextSize(value_buf);
    std::string exit = " : ";
    exit += value_buf;
    PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, max(it_hover->second, 0.0f)));
    RenderText(ImVec2(frame_bb.Min.x + label_size.x, total_bb.Min.y - 19), exit.c_str(), value_buf_end);
    PopStyleColor();
    // Slider grab

    GetWindowDrawList()->AddCircleFilled(ImVec2(max(frame_bb.Min.x + 3, grab_bb.Min.x * it_filled->second), grab_bb.Min.y + 3), 5, ImColor(41, 40, 46, int(255 * GetStyle().Alpha)));
    GetWindowDrawList()->AddCircle(ImVec2(max(frame_bb.Min.x + 3, grab_bb.Min.x * it_filled->second), grab_bb.Min.y + 3), 5, ImColor(51, 50, 56, int(255 * GetStyle().Alpha)));
    if (it_hover->second > 0.01)
        GetWindowDrawList()->AddCircleFilled(ImVec2(max(frame_bb.Min.x + 3, grab_bb.Min.x * it_filled->second), grab_bb.Min.y + 3), 4, ImColor(77, 125, 253, int(155 * it_hover->second)));
    PushStyleColor(ImGuiCol_Text, ImVec4(200 / 255.f, 200 / 255.f, 200 / 255.f, int(alpha_g * (100) + (155 * it_hover->second) / 255.f)));

    if (label_size.x > 0.0f)
        GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x, total_bb.Min.y - 19), ImColor(200, 200, 200, int((100 * alpha_g) + (155 * 1))), label);
    PopStyleColor();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
    return value_changed;
}
...
bool ImGui::tab(const char* icon, const char* label, bool selected, ImFont* icon_font, int current_tab)
{
    ...
    auto ga = GetStyle().Alpha;

    ImVec4 col = ImLerp(ImVec4{ 64 / 255.f, 64 / 255.f, 67 / 255.f, 0.9f }, ImVec4{ 180 / 255.f, 160 / 255.f, 240 / 255.f, ga }, it_filled->second);
    ImVec4 col2 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.9f }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, min(ga, 0.8f) }, it_filled->second);
    ImVec4 col3 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.9f }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 1.f }, it_filled->second);
    if (it_hover->second > 0.01 && !selected)
    {
        col = ImVec4(180 / 255.f, 160 / 255.f, 240 / 255.f, it_hover->second + 0.2f);
        col2 = ImVec4(180 / 255.f, 160 / 255.f, 240 / 255.f, it_hover->second - 0.2f);
    }
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 6, bb.Min.y - 6), ImVec2(bb.Max.x + 6, bb.Max.y + 6), ImColor(59, 59, 59, int(5 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 5, bb.Min.y - 5), ImVec2(bb.Max.x + 5, bb.Max.y + 5), ImColor(59, 59, 59, int(10 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 4, bb.Min.y - 4), ImVec2(bb.Max.x + 4, bb.Max.y + 4), ImColor(59, 59, 59, int(15 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 3, bb.Min.y - 3), ImVec2(bb.Max.x + 3, bb.Max.y + 3), ImColor(59, 59, 59, int(25 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 2, bb.Min.y - 2), ImVec2(bb.Max.x + 2, bb.Max.y + 2), ImColor(59, 59, 59, int(35 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 1, bb.Min.y - 1), ImVec2(bb.Max.x + 1, bb.Max.y + 1), ImColor(59, 59, 59, int(55 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(59, 59, 59, int(225 * it_filled->second)), 4);
    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(70, 70, 70, int(255 * it_filled->second)), 4);
    ...
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + icon_size.x / 2, bb.Min.y + size.y / 2 - icon_size.y / 2) + add, ImColor(170, 170, 170, int(200 * GetStyle().Alpha)), icon);
    if (selected)
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + icon_size.x / 2, bb.Min.y + size.y / 2 - icon_size.y / 2) + add, ImColor(77, 125, 253, int(255 * it_filled->second)), icon);
    PopFont();

    auto label_size = CalcTextSize(label);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 40, bb.Min.y + size.y / 2 - label_size.y / 2), ImColor(170, 170, 170, int(200 * GetStyle().Alpha)), label);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 40, bb.Min.y + size.y / 2 - label_size.y / 2), ImColor(200, 200, 200, int(255 * it_filled->second)), label);
...
bool ImGui::subtab(const char* label, bool selected)
{
    ...
    it_filled->second = clamp(it_filled->second + (2.15f * GetIO().DeltaTime * (selected ? 1.f : -1.2f)), 0.0f, 1.f);
    it_filled->second *= al;
    static std::map<ImGuiID, float> hover_animation;
    auto it_hover = hover_animation.find(id);
    if (it_hover == hover_animation.end())
    ...
    it_hover->second = clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 1.f);
    it_hover->second *= al;

    ImVec4 col = ImLerp(ImVec4{ 94 / 255.f, 94 / 255.f, 94 / 255.f, float(max(it_filled->second, 0.6) * al) }, ImVec4{ 130 / 255.f, 132 / 255.f, 170 / 255.f, float(max(it_filled->second, 0.6) * al) }, it_filled->second * al);
    ImVec4 col2 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, float(max(it_filled->second, 0.6) * al) }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, float(max(it_filled->second, 0.6) * al) }, it_filled->second * al);
    ImVec4 col3 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, float(max(it_filled->second, 0.6) * al) }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, float(max(it_filled->second, 0.6) * al) }, it_filled->second * al);

    if (hovered && !selected)
    {
        col = ImVec4(94 / 255.f, 94 / 255.f, 94 / 255.f, float(max(it_filled->second, 0.6) * al));
        col2 = ImVec4(32 / 255.f, 32 / 255.f, 35 / 255.f, float(max(it_filled->second, 0.6) * al));
    }

    GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(24, 24, 24, int(255 * al)), 6);

    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(3, 3), bb.Max + ImVec2(3, 3), ImColor(77, 125, 253, int((5 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), ImColor(77, 125, 253, int((15 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(77, 125, 253, int((25 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(125, 135, 255, int((205 * al) * it_filled->second)), 6);

    ImGui::PushFont(g_pMenuFont);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + (25 / 2 - label_size.y / 2)), ImColor(170, 170, 170, int(255 * al)), label);
    ImGui::PopFont();
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRect(pos - ImVec2(2, 2), p + size_arg + ImVec2(2, 2), ImColor(9, 9, 9, int(155 * alpha)), 5);
    parent_window->DrawList->AddRect(pos - ImVec2(1, 1), p + size_arg + ImVec2(1, 1), ImColor(10, 10, 10, int(155 * alpha)), 5);
    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(15, 15, 15, int(255 * alpha)), 5);
    parent_window->DrawList->AddRect(pos, p + size_arg, ImColor(24, 24, 24, int(255 * alpha)), 5);

    ImGui::PushFont(g_pMenuFont);
    parent_window->DrawList->AddText(pos + ImVec2(5, 1), ImColor(225, 225, 225, int(255 * alpha)), name);
    ImGui::PopFont();

    parent_window->DrawList->AddRect(p + ImVec2(1, 6), p + ImVec2(size_arg.x - 1, 6), ImColor(24, 24, 24, int(255 * alpha)));
