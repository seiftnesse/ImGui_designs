...
namespace ImGui
{
    ...
    IMGUI_API bool          Tab(const char* icon, const char* label, const bool active = false, const ImVec2& size_arg = ImVec2(0, 0));
    IMGUI_API bool          Bind(const char* label, int* k, const ImVec2& size_arg = ImVec2(0, 0));
    IMGUI_API bool          MultiCombo(const char* label, int current_item, const char* items[], int items_count, bool combos[]);
    /*                     EVOLUTION PROJECT GUI                     */
    ...
    IMGUI_API bool          MenuItem(const char* label, const char* shortcut, bool* p_selected, bool enabled = true);              // return true when activated + toggle (*p_selected) if p_selected != NULL
     ...
     // Menu
    IMGUI_API bool          subtab(const char* label, bool selected);
    IMGUI_API bool          tab(const char* icon, const char* label, bool selected, ImFont* icon_font, int current_tab);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_WaterWindow = 1 << 29,
    ImGuiWindowFlags_KeybindWindow = 1 << 29,   // Don't use! For internal use by BeginMenu()
