...
bool ImGui::subTab(const char* label, const ImVec2& size_arg, const bool selected)
{
    ...
    if (selected)
        window->DrawList->AddRectFilled({ bb.Min.x,bb.Min.y }, { bb.Max.x,bb.Max.y }, ImColor(25.f / 255.f, 25.f / 255.f, 25.f / 255.f, 255.f / 225.f));

    if (selected)
        window->DrawList->AddRectFilled({ bb.Max.x,bb.Max.y }, { bb.Max.x - 3,bb.Min.y }, ImColor(105 / 255.f, 125 / 255.f, 203 / 255.f, 255.f / 255.f));
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRect(pos - ImVec2(2, 2), p + size_arg + ImVec2(2, 2), ImColor(12, 12, 12, int(155 * alpha)), 5);
    parent_window->DrawList->AddRect(pos - ImVec2(1, 1), p + size_arg + ImVec2(1, 1), ImColor(19, 19, 19, int(155 * alpha)), 5);

    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(12, 12, 12, int(255 * alpha)), 5);

    parent_window->DrawList->AddRect(pos, p + size_arg, ImColor(18, 18, 18, int(255 * alpha)), 5);

    ImGui::PushFont(g_pMenuFont);
    parent_window->DrawList->AddText(pos + ImVec2(5, 6), ImColor(225, 225, 225, int(255 * alpha)), name);
    ImGui::PopFont();

    parent_window->DrawList->AddRect(p + ImVec2(1, 7), p + ImVec2(size_arg.x - 1, 7), ImColor(105, 125, 203, int(255 * alpha)));
...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(pos, ImVec2(pos.x + 65, pos.y + 50));
    ...
    ImVec4 text = ImLerp(ImVec4{ 100 / 255.f, 125 / 255.f, 208 / 255.f, GetStyle().Alpha }, ImVec4{ 100 / 255.f, 125 / 255.f, 208 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImVec4 text2 = ImLerp(ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, GetStyle().Alpha }, ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, GetStyle().Alpha }, it_filled->second);

    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 0, bb.Min.y + 0), ImVec2(bb.Min.x + 65, bb.Max.y + 10), ImColor(25, 25, 25, int(225 * it_filled->second)), 4);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 18 , bb.Min.y + 8), ImColor(text), icon);
