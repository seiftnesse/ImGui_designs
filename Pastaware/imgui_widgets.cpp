...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(5, 8), 3, ImColor(132, 136, 139), 360);
    ...
    else if (*v)
    {
        ...
        ImGui::GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(5, 8), 3, ImColor(201, 239, 8), 360);
    }
    ...
    if (label_size.x > 0.0f)
        if (*v) {
            ...
            ImGui::PushStyleColor(ImGuiCol_Text, rgb_to_imvec4(132, 136, 139, 255));
            RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label);
            ImGui::PopStyleColor();
        }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 65), window->DC.CursorPos - ImVec2(0, -65) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(0, 2), ImVec2(value_x2, frame_bb.Max.y) + ImVec2(0, 2), ImColor(55, 59, 62), 5);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y) - ImVec2(8, 2), frame_bb.Max + ImVec2(0, 2), ImColor(55, 59, 62), 5);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x && !popup_open)
            ImGui::GetWindowDrawList()->AddCircleFilled(ImVec2(value_x2 + 5, frame_bb.Min.y + 8), 4, ImColor(132, 136, 139), 360);
        else
            ImGui::GetWindowDrawList()->AddCircleFilled(ImVec2(value_x2 + 5, frame_bb.Min.y + 8), 4, ImColor(201, 239, 8), 360);
    }
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview && !popup_open)) {
        ImGui::PushStyleColor(ImGuiCol_Text, rgb_to_imvec4(132, 136, 139, 255));
        ImGui::PushFont(ComboPrew);
        RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(2, 1), ImVec2(value_x2, frame_bb.Max.y) - ImVec2(2, 1), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
        ImGui::PopStyleColor();
        ImGui::PopFont();
    }
    else {
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 141, frame_bb.Min.y + style.FramePadding.y - 20), label);
    ImGui::PushFont(ComboPrew);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 30), window->DC.CursorPos - ImVec2(0, -30) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(0, 5), ImColor(55, 59, 62), 5);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(-0.5, 4.5), grab_bb.Max - ImVec2(-0.5, 4.5), ImColor(201, 239, 8), 7);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(frame_bb.Min - ImVec2(0, 28), frame_bb.Max + ImVec2(95, 0), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 141, frame_bb.Min.y + style.FramePadding.y - 14), label);
