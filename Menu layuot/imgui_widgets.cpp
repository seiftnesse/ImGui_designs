...
bool ImGui::Checkbox(const char* label, bool* v, bool offset)
{
    ...
    it_filled->second = ImClamp(it_filled->second + (2.0f * ImGui::GetIO().DeltaTime * ((*v) ? 1.f : -1.4f)), it_hover->second, 1.f);
    ...
    ImVec4 text = ImLerp(ImVec4{ 139 / 255.f, 139 / 255.f, 146 / 255.f, aslpha }, ImVec4{ 232 / 255.f, 232 / 255.f, 232 / 255.f, aslpha }, it_filled->second);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(10, 10, 10, int (255 * aslpha)), 2);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(130 / 255.f, 132 / 255.f, 170 / 255.f, it_filled->second), 2);

    
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(6, 5), ImColor(14, 14, 26, int(255 * it_filled->second)), 8);


    if (label_size.x > 0.0f) {
        GetWindowDrawList()->AddText(text_bb.GetTL(), ImColor(text), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(27,  0), window->DC.CursorPos + ImVec2(0, 0) + ImVec2(window->Size.x - 28, label_size.y + style.FramePadding.y * 2.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(24, 0), window->DC.CursorPos + ImVec2(window->Size.x - 24, label_size.y + 16));
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(10, 10, 10, (int)(g.Style.Alpha * 255)), 2);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y), ImColor(130 / 255.f, 132 / 255.f, 170 / 255.f, GetStyle().Alpha), 2, ImDrawFlags_RoundCornersLeft);

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y - 4), 6, ImColor(18, 18, 18, int(255 * GetStyle().Alpha)));
    window->DrawList->AddCircle(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y - 4), 6, ImColor(20 / 255.f, 20 / 255.f, 20 / 255.f, GetStyle().Alpha));
    if (it_hover->second > 0.01)
        window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y - 4), 5, ImColor(130 / 255.f, 132 / 255.f, 170 / 255.f, it_hover->second), 160);
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
