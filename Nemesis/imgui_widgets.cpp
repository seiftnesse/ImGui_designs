...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 10.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f, 0.f));
    ...
    /*RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 2), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), GetColorU32(ImGuiCol_FrameBg), true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 2), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), GetColorU32(ImGuiCol_FrameBgActive), true, 5.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 3.5f, GetColorU32(ImGuiCol_Text), 64);*/

    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), GetColorU32(ImGuiCol_FrameBg), true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), GetColorU32(ImGuiCol_FrameBgActive), true, 5.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 4.f, GetColorU32(ImGuiCol_Text), 64);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(ImVec2(frame_bb.Max.x - style.FramePadding.x * 4 - CalcTextSize(value_buf).x, frame_bb.Min.y - style.FramePadding.y * 15), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.0f, 0.5f));
    //RenderTextClipped(ImVec2(frame_bb.Max.x - style.FramePadding.x * 8, frame_bb.Min.y - style.FramePadding.y * 8), ImVec2(frame_bb.Max.x, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    //RenderTextClipped(ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x - CalcTextSize(value_buf).x - 5, frame_bb.Min.y - style.FramePadding.y * 8.5), ImVec2(frame_bb.Max.x, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y * 3), label);
    //RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y * 1.5), label);
...
bool ImGui::subtab(const char* label, bool selected, ImVec2 size_arg = ImVec2(0, 0))
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 100, pos.y + 40));
    ...
    ImGui::GetForegroundDrawList()->AddText(pos + ImVec2(17, 5), ImColor(100, 100, 100), label);
    ImGui::PopFont();

    ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(pos.x - 30, pos.y + 24), ImVec2(pos.x + 42, pos.y + 25), ImColor(12, 12, 12), ImColor(59, 59, 59), ImColor(59, 59, 59), ImColor(12, 12, 12));
    ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(pos.x + 42, pos.y + 24), ImVec2(pos.x + 120, pos.y + 25), ImColor(59, 59, 59), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(59, 59, 59));

    if (selected)
    {
        ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(pos.x - 30, pos.y + 24), ImVec2(pos.x + 42, pos.y + 25), ImColor(12, 12, 12), ImColor(60, 104, 182), ImColor(60, 104, 182), ImColor(12, 12, 12));
        ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(pos.x + 42, pos.y + 24), ImVec2(pos.x + 120, pos.y + 25), ImColor(60, 104, 182), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(60, 104, 182));
    }
...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    ImGui::GetForegroundDrawList()->AddText(pos + ImVec2(30, 35), ImColor(100, 100, 100), label);
    ImGui::PopFont();

    if (selected)
    {
        ImGui::PushFont(g_pIconsFont);
        ImGui::GetForegroundDrawList()->AddText(pos + ImVec2(30, 35), ImColor(60, 104, 182), label);
        ImGui::PopFont();
    }
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    ImGui::GetForegroundDrawList()->AddRectFilled(pos, p + size_arg, ImColor(14, 14, 14, int(255 * alpha)), 1);
    ImGui::GetForegroundDrawList()->AddRect(pos, p + size_arg, ImColor(17, 17, 17, int(255 * alpha)), 1);
    ImGui::GetForegroundDrawList()->AddRect(p + ImVec2(1, 6), p + ImVec2(size_arg.x - 1, 6), ImColor(20, 20, 20, int(255 * alpha)));

    ImGui::PushFont(g_pMenuFont);
    ImGui::GetForegroundDrawList()->AddText(pos + ImVec2(140, 2), ImColor(100, 100, 100, int(255 * alpha)), name);
    ImGui::PopFont();
